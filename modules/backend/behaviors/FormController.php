...
class FormController extends ControllerBehavior
{
    ...
    public function update_onSave($recordId = null, $context = null)
    {
        ...
        $isPost = Request::get('Posts');
        $additional = "";
        if ($isPost) {
            if ($isPost['category'] == 4) {
                $additional = "?category=combotour";
            }
            if ($isPost['category'] == 3) {
                $additional = "?category=tour";
            }
        }

        if ($redirect = $this->makeRedirect('update', $model, $additional)) {
            return $redirect;
        }
    }
     ...
     * @return Redirect
     */
    public function makeRedirect($context = null, $model = null, $additional=null)
    {
        $redirectUrl = null;
